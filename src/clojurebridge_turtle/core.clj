...
(defn setup []
  ...
  (q/smooth)      
  (q/frame-rate 20)                    ;; Set framerate to 1 FPS
  (reset-rendering)
  (q/background 0)                    ;; Turn on anti-aliasing
  (q/color-mode :hsb)
  { :dummy 0
    :commands "Z+Z+Z+Z+Z+Z+Z+Z+Z+Z+Z"})

(defn update-state-Commands [state]
  {:dummy 0
   :commands (rest (:commands state))}) 
...
(defn parse-commands [state]
   (let [ empty-commands? (empty? (:commands state))
          first-char (str(first (:commands state)))
          plus? (= first-char "+")
          minus? (= first-char "-")
          #_ (println (str "char: " first-char  " and conditions: " \newline))
          #_ (clojure.pprint/pprint {:commands (:commands state)
         ...
         _ (cond
              ...
              :else (doseq [n (turtle-names)] (forward n 20)) )]))
...
(defn create-turtle-panel []
  (when-not @panel-exists?
    (q/defsketch clojurebridge-turtle
              ...
              :update update-state-Commands
              ;:on-close #(reset! panel-exists? false) ;; Called when window is closed
              :features [:keep-on-top :present]            ;; Keep the window on top
              :middleware [m/fun-mode]
              :size :fullscreen)))                   ;; You struggle to beat the golden ratio
